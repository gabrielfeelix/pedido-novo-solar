...
interface PedidoContextValue extends PedidoState {
  ...
  updateLooseItemQuantity: (itemId: string, quantity: number) => void;
...
export function PedidoProvider({ children }: { children: ReactNode }) {
  ...
  const updateLooseItemQuantity = (itemId: string, quantity: number) => {
    const nextQuantity = Math.max(1, Math.floor(quantity));
    setState((s) => ({
      ...s,
      orderItems: s.orderItems.map((item) => {
        if (item.type !== 'loose' || item.id !== itemId) return item;
        return {
          ...item,
