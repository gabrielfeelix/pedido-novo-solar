...
export function ProductListing() {
  ...
  const flatRows = useMemo(() => {
    ...
    allProducts.forEach((p) => {
      ...
      if (estoqueFilter === 'sem' && !p.filiais.some((f) => f.estoque === 0)) return;
      if (estoqueFilter === 'com' && p.filiais.some((f) => f.estoque === 0)) return;
      if (estoqueFilter === 'critico') {
        const total = p.filiais.reduce((s, f) => s + f.estoque, 0);
        if (total > 50) return;
      }
      if (agingFilter) {
