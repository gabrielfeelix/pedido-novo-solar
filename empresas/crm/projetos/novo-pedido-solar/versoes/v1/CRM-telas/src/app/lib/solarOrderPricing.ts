import type { OrderItem, SaleType } from '../context/PedidoContext';
...
function baseGrossForItem(item: OrderItem) {
...
export function buildDefaultLineAdjustment(item: OrderItem): LinePricingAdjustment {
  return {
    discountMode: 'percent',
    discountValue: item.type === 'generator' ? DEFAULT_DISCOUNT_PCT : 0,
    prizeMode: item.type === 'generator' ? item.prizeMode : 'percent',
    prizeValue: item.type === 'generator' ? item.prizeValue : 0,
  };
}
