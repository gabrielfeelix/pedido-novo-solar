...
function MainStepIndicator({ currentNfStep, subStep, accentColor }: { currentNfStep: NfStep; subStep: SubStepKey; accentColor?: string }) {
    ...
    <div className="flex items-center justify-center flex-wrap">
      {steps.map((step, i) => (
        <div key={step.key} className="flex items-center">
          ...
          {i < steps.length - 1 && (
            <div
              className="h-[2px] mx-2 sm:mx-3 mb-[22px] rounded-full transition-colors duration-300"
              style={{ width: 44, background: step.done ? accent : 'rgba(255,255,255,0.18)' }}
            />
          )}
        </div>
      ))}
...
export function CheckoutPage() {
  ...
  /* ── V2: sub-steps dentro do checkout do pedido (Frete → Pagamento → Detalhes).
       Quando user troca de filial, volta pra frete. ── */
  type SubStep = 'frete' | 'pagamento' | 'detalhes';
  const [subStep, setSubStep] = useState<SubStep>('frete');
  ...
  /* ── Validation per sub-step (V2 wizard) ── */
  const canProceedSubStep = (filial: 'PR' | 'ES', sub: SubStep): boolean => {
    const s = nfState[filial];
    if (sub === 'frete') return s.selectedShipping !== null;
    if (sub === 'pagamento') {
      const nfTotalAfter = filial === 'PR' ? totalPRAfterCredit : totalESAfterCredit;
      const creditCoversNf = nfTotalAfter === 0;
      if (creditCoversNf) return true;
      return s.paymentMethod !== null && s.condicaoPgto !== '';
    }
    return true; // detalhes (consultor + obs) sempre passa
  };

  /* ── Navigation V2 — wizard sub-step → filial → review ── */
  const handleSaveAndContinue = () => {
    if (nfStep === 'review') return;
    if (subStep === 'frete')      { setSubStep('pagamento'); return; }
    if (subStep === 'pagamento')  { setSubStep('detalhes');  return; }

    // subStep === 'detalhes' → próxima filial OU revisão
  ...
  const handleBack = () => {
    ...
    if (subStep === 'detalhes')  { setSubStep('pagamento'); return; }
    if (subStep === 'pagamento') { setSubStep('frete');     return; }

    // subStep === 'frete' → filial anterior OU carrinho
    const currentFilialIdx = activeFiliaisInCart.indexOf(nfStep as 'PR' | 'ES');
    ...
    <div className="min-h-screen" style={{ background: 'var(--background)', fontFamily: 'var(--font-red-hat-display)' }}>
      ...
      <div className="relative overflow-hidden" style={{ background: 'var(--foreground)' }}>
        ...
        <div className="relative max-w-[1280px] mx-auto px-4 md:px-6 pt-6 pb-7 text-center">
          <MainStepIndicator currentNfStep={nfStep} subStep={subStep} accentColor={NF_STYLE[activeFilial].color} />
      ...
      <div className="max-w-[1280px] mx-auto flex flex-col lg:flex-row gap-8 pt-5 pb-16 px-4 md:px-6">

        {/* Left column — wizard sub-steps */}
        <div className="flex-1 min-w-0 flex flex-col gap-5">

          {/* ── Guiding header — frase amigável muda por sub-step ── */}
          <div className="mb-1">
            <span className="block" style={{ fontSize: 'var(--text-2xs)', fontWeight: 'var(--font-weight-bold)', color: 'var(--primary)', fontFamily: 'var(--font-red-hat-display)', letterSpacing: '1.5px', textTransform: 'uppercase' }}>
              {subStep === 'frete' ? 'PASSO 1 DE 3' : subStep === 'pagamento' ? 'PASSO 2 DE 3' : 'PASSO 3 DE 3'}
            </span>
            <h3 className="m-0 mt-1.5" style={{ fontSize: 'clamp(22px, 3vw, 28px)', fontWeight: 'var(--font-weight-bold)', color: 'var(--foreground)', fontFamily: 'var(--font-red-hat-display)', lineHeight: 1.15 }}>
              {subStep === 'frete' && <>Vamos começar pela entrega.<br /><span style={{ color: 'var(--primary)' }}>Como você quer receber seu pedido?</span></>}
              {subStep === 'pagamento' && <>Agora, vamos ao pagamento.<br /><span style={{ color: 'var(--primary)' }}>Como prefere pagar?</span></>}
              {subStep === 'detalhes' && <>Quase lá!<br /><span style={{ color: 'var(--primary)' }}>Ajuste os últimos detalhes do pedido.</span></>}
            </h3>
            <p className="mt-2 mb-0" style={{ fontSize: 'var(--text-sm)', color: 'var(--muted-foreground)', fontFamily: 'var(--font-red-hat-display)', lineHeight: 1.5 }}>
              {subStep === 'frete' && 'Escolha a logística que melhor se encaixa: entrega CIF, transportadora FOB ou retirada na filial.'}
              {subStep === 'pagamento' && 'Use seu crédito disponível, escolha o método e a condição que cabem no seu fluxo.'}
              {subStep === 'detalhes' && 'Confirme se quer falar com seu consultor e adicione observações do pedido ou NF.'}
            </p>
          </div>
          ...
          {/* ── PASSO 2: Pagamento (crédito + forma de pagamento) ── */}
          {subStep === 'pagamento' && (
            <>
              {renderCreditBlock()}
              <AnimatePresence mode="wait">
          ...
          {/* ── PASSO 3: Detalhes (consultor + observações) ── */}
          {subStep === 'detalhes' && (
            <>
          <div
            className="rounded-xl overflow-hidden"
          ...
          {/* ── V2 Footer navigation — Voltar / Próximo wizard ── */}
          {(() => {
            const stepCanProceed = canProceedSubStep(activeFilial, subStep);
            const isLastSub = subStep === 'detalhes';
            const backLabel = subStep === 'frete'
              ? (currentIdx === 0 ? '← Voltar ao Carrinho' : `← Editar Filial ${activeFiliaisInCart[currentIdx - 1]}`)
              : subStep === 'pagamento' ? '← Voltar ao Frete'
              : '← Voltar ao Pagamento';
            const nextLabel = subStep === 'frete' ? 'Próximo — Pagamento →'
              : subStep === 'pagamento' ? 'Próximo — Detalhes →'
              : (isLastNf ? 'Revisar e Finalizar →' : `Salvar e configurar Filial ${activeFiliaisInCart[currentIdx + 1]} →`);
            return (
              <>
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-4">
                  <button onClick={handleBack}
                    className="h-12 px-8 rounded-xl cursor-pointer hover:opacity-80 transition-opacity order-2 sm:order-1 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2"
                    style={{ background: 'transparent', border: '1px solid var(--muted)', color: 'var(--foreground)', fontSize: 'var(--text-sm)', fontWeight: 'var(--font-weight-bold)', fontFamily: 'var(--font-red-hat-display)', outlineColor: 'var(--primary)' }}>
                    {backLabel}
                  </button>
                  <button
                    onClick={handleSaveAndContinue}
                    disabled={!stepCanProceed}
                    className="h-12 px-8 rounded-xl border-none cursor-pointer hover:opacity-90 transition-all disabled:opacity-40 disabled:cursor-not-allowed order-1 sm:order-2 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2"
                    style={{ background: nfStyle.color, color: 'var(--primary-foreground)', fontSize: 'var(--text-sm)', fontWeight: 'var(--font-weight-bold)', fontFamily: 'var(--font-red-hat-display)', outlineColor: nfStyle.color }}>
                    {nextLabel}
                  </button>
                </div>
                {!stepCanProceed && (
                  <div className="flex items-start gap-2 -mt-1">
                    ...
                    <span className="block" style={{ fontSize: 'var(--text-xs)', color: 'var(--muted-foreground)', fontFamily: 'var(--font-red-hat-display)', lineHeight: '1.5' }}>
                      {subStep === 'frete'
                        ...
                        : 'Selecione forma e condição de pagamento para continuar.'}
                    </span>
                  </div>
                )}
              </>
